...
export default function HouseScreen({ navigation }) {
  ...
  const getHouses = async () => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/type/House`);
      const data = await response.json();

      setHouses(data);
    } catch (error) {
      console.log("gagal mengambil data house:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener("focus", getHouses);
    return unsubscribe;
  }, [navigation]);
    ...
    <View style={styles.container}>
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>Data house masih kosong</Text>
          }
        />
      )}
